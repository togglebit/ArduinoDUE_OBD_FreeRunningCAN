...
class CANRaw
{
  ...
  private:
	...
	void (*cbCANFrame[9])(RX_CAN_FRAME *); //8 mailboxes plus an optional catch all
  ...
  public:
	...
	bool sendFrame(TX_CAN_FRAME& txFrame);
	...
	void setCallback(int mailbox, void (*cb)(RX_CAN_FRAME *));
	void setGeneralCallback(void (*cb)(RX_CAN_FRAME *));
	//note that these below versions still use mailbox number. There isn't a good way around this. 
	void attachCANInterrupt(void (*cb)(RX_CAN_FRAME *)); //alternative callname for setGeneralCallback
	void attachCANInterrupt(uint8_t mailBox, void (*cb)(RX_CAN_FRAME *));
